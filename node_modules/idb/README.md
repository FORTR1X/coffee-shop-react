...
This is a tiny (~1.05k brotli'd) library that mostly mirrors the IndexedDB API, but with small improvements that make a
...
import `idb/with-async-ittr` instead of `idb` (this increases the library size to ~1.28k brotli'd):
